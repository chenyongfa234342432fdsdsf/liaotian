...
class Generator {
  ...
  async generate() {
    ...
    await Promise.all(this.config.map(async (serverConfig, serverIndex) => {
      const projects = serverConfig.projects.reduce((projects, project) => {
        projects.push(...(0, _vtils.castArray)(project.token).map(token => ({
          ...project,
          token: token
        })));
        return projects;
      ...
      return Promise.all(projects.map(async (projectConfig, projectIndex) => {
        ...
        await Promise.all(projectConfig.categories.map(async (categoryConfig, categoryIndex) => {
          // 分类处理
          // 数组化
          let categoryIds = (0, _vtils.castArray)(categoryConfig.id);
          // 全部分类
          if (categoryIds.includes(0)) {
            categoryIds.push(...projectInfo.cats.map(cat => cat._id));
          }
          // 唯一化
          categoryIds = (0, _vtils.uniq)(categoryIds);
          // 去掉被排除的分类
          ...
          const codes = (await Promise.all(categoryIds.map(async (id, categoryIndex2) => {
            ...
            interfaceList = interfaceList.map(interfaceInfo => {
              // 实现 _project 字段
              interfaceInfo._project = (0, _vtils.omit)(projectInfo, ['cats', 'getMockUrl', 'getDevUrl', 'getProdUrl']);
              // 预处理
              const _interfaceInfo = (0, _vtils.isFunction)(syntheticalConfig.preproccessInterface) ? syntheticalConfig.preproccessInterface((0, _vtils.cloneDeepFast)(interfaceInfo), changeCase, syntheticalConfig) : interfaceInfo;
  ...
  async write(outputFileList) {
    return Promise.all(Object.keys(outputFileList).map(async outputFilePath => {
      ...
      if (!syntheticalConfig.typesOnly) {
        ...
        if (syntheticalConfig.reactHooks && syntheticalConfig.reactHooks.enabled && !(await _fsExtra.default.pathExists(rawRequestHookMakerFilePath))) {
          await _fsExtra.default.outputFile(requestHookMakerFilePath, (0, _vtils.dedent)`
                ...
                import type { Request } from ${JSON.stringify((0, _utils.getNormalizedRelativePath)(requestHookMakerFilePath, outputFilePath))}
      ...
      const prettyOutputContent = _prettier.default.format(rawOutputContent, {
        ...(await (0, _utils.getCachedPrettierOptions)()),
      ...
      const outputContent = `${(0, _vtils.dedent)`
          ...
          /* prettier-ignore-end */
        `}\n`;
      await _fsExtra.default.outputFile(outputFilePath, outputContent);
  ...
  async tsc(file) {
    return new Promise(resolve => {
      ...
      (0, _child_process.exec)(`${command} --target ES2019 --module ESNext --jsx preserve --declaration --esModuleInterop ${JSON.stringify(file)}`, {
  ...
  async fetchApi(url, query) {
    const res = await (0, _utils.httpGet)(url, query);
  ...
  async generateInterfaceCode(syntheticalConfig, interfaceInfo, categoryUID) {
    ...
    let responseDataType = await (0, _utils.jsonSchemaToType)(responseDataJsonSchema, responseDataTypeName);
    ...
    // **/
    // export const ${interfaceInfo.method.toLowerCase()}${requestDataTypeName.replace(/Yapi/i, '').replace((0, _utils.capitalizeFirstLetter)(interfaceInfo.method), '')}: MarkcoinRequest<
    //   ${requestDataTypeName},
    ...
    const requestHookName = syntheticalConfig.reactHooks && syntheticalConfig.reactHooks.enabled ? (0, _vtils.isFunction)(syntheticalConfig.reactHooks.getRequestHookName) ? /* istanbul ignore next */
    ...
    return (0, _vtils.dedent)`
      ...
      return res;
              ...
              requestBodyType: RequestBodyType.${extendedInterfaceInfo.method === _types.Method.GET ? _types.RequestBodyType.query : extendedInterfaceInfo.req_body_type /* istanbul ignore next */ || _types.RequestBodyType.none},
              ...
              queryStringArrayFormat: QueryStringArrayFormat.${syntheticalConfig.queryStringArrayFormat || _types.QueryStringArrayFormat.brackets},
