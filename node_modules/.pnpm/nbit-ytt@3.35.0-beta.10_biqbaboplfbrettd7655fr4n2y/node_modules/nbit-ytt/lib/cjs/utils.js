...
function traverseJsonSchema(jsonSchema, cb, currentPath = []) {
  /* istanbul ignore if */
  if (!(0, _vtils.isObject)(jsonSchema)) return jsonSchema;
  ...
  if (jsonSchema.properties) {
    (0, _vtils.forOwn)(jsonSchema.properties, (item, key) => traverseJsonSchema(item, cb, [...currentPath, key]));
  }

  // 继续处理数组的子元素
  if (jsonSchema.items) {
    (0, _vtils.castArray)(jsonSchema.items).forEach((item, index) => traverseJsonSchema(item, cb, [...currentPath, index]));
  }
...
function jsonSchemaToJSTTJsonSchema(jsonSchema, typeName) {
  ...
  return traverseJsonSchema(jsonSchema, (jsonSchema, currentPath) => {
    ...
    if (refValue != null && refValue.startsWith('&')) {
      ...
      const typeAbsolutePath = toUnixPath(_path.default.resolve(_path.default.dirname(`/${currentPath.join('/')}`.replace(/\/{2,}/g, '/')), typeRelativePath).replace(/^[a-z]+:/i, ''));
    ...
    Array.isArray(currentPath) && currentPath.length > 0 && jsonSchema.type === 'object' && jsonSchema.properties && Object.keys(jsonSchema.properties).length > 0) {
      let aggreagedName = typeName.replaceAll(/ApiResponse/gi, '').replaceAll(/list/gi, '').replaceAll(/data/gi, '');
      ...
      jsonSchema.title = capitalizeFirstLetterOnly(aggreagedName.replaceAll(/datadata/gi, 'Data').replaceAll(/listlist/gi, 'List'));
...
function mockjsTemplateToJsonSchema(template, customTypeMapping) {
  ...
  (0, _vtils.traverse)(template, (value, key, parent) => {
...
async function jsonSchemaToType(jsonSchema, typeName) {
  ...
  const code = await (0, _jsonSchemaToTypescript.compile)(jsonSchemaToJSTTJsonSchema((0, _vtils.cloneDeepFast)(jsonSchema), typeName), fakeTypeName, JSTTOptions);
  const replced = code.replace(fakeTypeName, typeName).trim().replaceAll(/export interface (?!Yapi)/gi, 'export interface Yapi');
  return replced;
}
