...
function openapi2swagger(data) {
  data.swagger = '2.0';
  (0, _vtils.each)(data.paths, apis => {
    (0, _vtils.each)(apis, api => {
      (0, _vtils.each)(api.responses, res => {
...
async function handleSwaggerData(res) {
  return new Promise(resolve => {
    const data = (0, _swaggerClient.default)({
      spec: res
    });
    data.then(res => {
      resolve(res.spec);
...
async function run(res) {
  ...
  (0, _vtils.each)(res.paths, (apis, path) => {
    ...
    (0, _vtils.each)(apis, (api, method) => {
      ...
      try {
        ...
        if (data.catname) {
          if (!(0, _vtils.find)(interfaceData.cats, item => item.name === data.catname)) {
  ...
  interfaceData.cats = interfaceData.cats.filter(catData => {
    ...
    return (0, _vtils.find)(interfaceData.apis, apiData => {
...
function handleSwagger(data, originTags = []) {
  ...
  if (data.tags && Array.isArray(data.tags)) {
    ...
    for (let i = 0; i < data.tags.length; i++) {
      ...
      if (originTags.length > 0 && (0, _vtils.find)(originTags, item => {
...
async function swaggerJsonToYApiData(data) {
  ...
  const currentTime = (0, _dayjs.default)().unix();
  const project = {
    _id: 0,
  ...
  const interfaces = yapiData.apis.map((api, index) => {
    ...
    return {
      ...
      _id: index + 1,
      project_id: 0,
      catid: ((_cats$find = cats.find(cat => cat.name === api.catname)) == null ? void 0 : _cats$find._id) || -1,
      tag: api.tag || [],
      add_time: currentTime,
      up_time: currentTime
    };
