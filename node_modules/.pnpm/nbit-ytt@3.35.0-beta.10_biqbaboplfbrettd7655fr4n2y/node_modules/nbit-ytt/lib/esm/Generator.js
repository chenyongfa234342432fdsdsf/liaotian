...
import { capitalizeFirstLetter, getCachedPrettierOptions, getNormalizedRelativePath, getRequestDataJsonSchema, getResponseDataJsonSchema, httpGet, jsonSchemaToType, sortByWeights, throwError } from './utils';
...
export class Generator {
  ...
  constructor(config, options = {
    ...
    this.config = [];
    this.disposes = [];
    this.fetchProject = memoize(async ({
      serverUrl,
      token
    }) => {
    ...
    this.fetchExport = memoize(async ({
      serverUrl,
      token
    }) => {
      ...
      return categoryList.map(cat => {
        ...
        const projectId = ((_cat$list = cat.list) == null ? void 0 : (_cat$list$ = _cat$list[0]) == null ? void 0 : _cat$list$.project_id) || 0;
        const catId = ((_cat$list2 = cat.list) == null ? void 0 : (_cat$list2$ = _cat$list2[0]) == null ? void 0 : _cat$list2$.catid) || 0;
        // 实现分类在 YApi 上的地址
  ...
  async generate() {
    ...
    await Promise.all(this.config.map(async (serverConfig, serverIndex) => {
      const projects = serverConfig.projects.reduce((projects, project) => {
        projects.push(...castArray(project.token).map(token => ({
          ...project,
          token: token
        })));
        return projects;
        ...
        await Promise.all(projectConfig.categories.map(async (categoryConfig, categoryIndex) => {
          ...
          const codes = (await Promise.all(categoryIds.map(async (id, categoryIndex2) => {
            ...
            interfaceList = interfaceList.map(interfaceInfo => {
              ...
              // 预处理
              const _interfaceInfo = isFunction(syntheticalConfig.preproccessInterface) ? syntheticalConfig.preproccessInterface(cloneDeepFast(interfaceInfo), changeCase, syntheticalConfig) : interfaceInfo;
            ...
            const interfaceCodes = await Promise.all(interfaceList.map(async interfaceInfo => {
              const outputFilePath = path.resolve(this.options.cwd, typeof syntheticalConfig.outputFilePath === 'function' ? syntheticalConfig.outputFilePath(interfaceInfo, changeCase) : syntheticalConfig.outputFilePath);
            ...
            return Object.keys(groupedInterfaceCodes).map(outputFilePath => {
              const categoryCode = [...uniq(sortByWeights(groupedInterfaceCodes[outputFilePath]).map(item => item.categoryUID)).map(categoryUID => syntheticalConfig.typesOnly ? '' : dedent`
                                    ...
                                    `), ...sortByWeights(groupedInterfaceCodes[outputFilePath]).map(item => item.code)].filter(Boolean).join('\n\n');
              if (!outputFileList[outputFilePath]) {
                outputFileList[outputFilePath] = {
                  ...
                  requestHookMakerFilePath: syntheticalConfig.reactHooks && syntheticalConfig.reactHooks.enabled ? syntheticalConfig.reactHooks.requestHookMakerFilePath ? path.resolve(this.options.cwd, syntheticalConfig.reactHooks.requestHookMakerFilePath) : path.join(path.dirname(outputFilePath), 'makeRequestHook.ts') : ''
                };
              }
              return {
    ...
    return Promise.all(Object.keys(outputFileList).map(async outputFilePath => {
      ...
      if (!syntheticalConfig.typesOnly) {
        if (!(await fs.pathExists(rawRequestFunctionFilePath))) {
          await fs.outputFile(requestFunctionFilePath, dedent`
        ...
        if (syntheticalConfig.reactHooks && syntheticalConfig.reactHooks.enabled && !(await fs.pathExists(rawRequestHookMakerFilePath))) {
          await fs.outputFile(requestHookMakerFilePath, dedent`
      ...
      if (syntheticalConfig.target === 'javascript') {
        ...
        await Promise.all([fs.remove(requestFunctionFilePath).catch(noop), fs.remove(requestHookMakerFilePath).catch(noop), fs.remove(outputFilePath).catch(noop)]);
      }
    }));
  }
  async tsc(file) {
  ...
  async fetchInterfaceList({
    ...
    const category = ((await this.fetchExport({
      serverUrl,
      token
    })) || []).find(cat => !isEmpty(cat) && !isEmpty(cat.list) && cat.list[0].catid === id);
    if (category) {
  ...
  async generateInterfaceCode(syntheticalConfig, interfaceInfo, categoryUID) {
    ...
    let responseDataType = await jsonSchemaToType(responseDataJsonSchema, responseDataTypeName);
    ...
    const requestHookName = syntheticalConfig.reactHooks && syntheticalConfig.reactHooks.enabled ? isFunction(syntheticalConfig.reactHooks.getRequestHookName) ? /* istanbul ignore next */
    await syntheticalConfig.reactHooks.getRequestHookName(extendedInterfaceInfo, changeCase) : `use${changeCase.pascalCase(requestFunctionName)}` : '';
    ...
    const genComment = genTitle => {
      ...
      const summary = [hasCategory && {
        ...
        ? String(extendedInterfaceInfo.up_time) : /* istanbul ignore next */
    ...
    // 请求参数额外信息
    const requestFunctionExtraInfo = typeof syntheticalConfig.setRequestFunctionExtraInfo === 'function' ? await syntheticalConfig.setRequestFunctionExtraInfo(extendedInterfaceInfo, changeCase) : {};
    return dedent`
      ...
      return res;
              ...
              requestBodyType: RequestBodyType.${extendedInterfaceInfo.method === Method.GET ? RequestBodyType.query : extendedInterfaceInfo.req_body_type /* istanbul ignore next */ || RequestBodyType.none},
